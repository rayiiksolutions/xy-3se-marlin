  ...
  if (encoder_diffState == ENCODER_DIFF_CW)
    ...
    if (select_print.now == 1) { // pause window
      if (HMI_flag.select_flag) {
        ...
        ICON_Continue();
        #if ENABLED(POWER_LOSS_RECOVERY)
          if (PrintJobRecovery::enabled) PrintJobRecovery::save(true);
        #endif
...
void EachMomentUpdate() {
  ...
  if (checkkey == PrintProcess) {
    ...
    if (HMI_flag.print_finish && !HMI_flag.done_confirm_flag) {
      ...
      TERN_(POWER_LOSS_RECOVERY, PrintJobRecovery::cancel());
  ...
  if (card.isPrinting() && checkkey == PrintProcess) { // print process
    ...
    Goto_MainMenu();
  ...
  #if ENABLED(POWER_LOSS_RECOVERY)
    else if (DWIN_lcd_sd_status && PrintJobRecovery::dwin_flag) { // resume print before power off
      static bool recovery_flag = false;

      PrintJobRecovery::dwin_flag = false;
      recovery_flag = true;
      ...
      //
      //(void)PrintJobRecovery::interrupted_file_exists();
      char * const name = card.longest_filename();
      const int8_t npos = _MAX(0U, DWIN_WIDTH - strlen(name) * (MENU_CHR_W)) / 2;
