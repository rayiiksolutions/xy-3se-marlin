...
void printer_state_polling() {
  ...
  if (uiCfg.print_state == RESUMING) {
    ...
    if (uiCfg.print_state == REPRINTED) {
      ...
      #if HAS_HOTEND
        HOTEND_LOOP() {
          const int16_t et = PrintJobRecovery::info.target_temperature[e];
          if (et) {
            #if HAS_MULTI_HOTEND
      ...
      PrintJobRecovery::resume();
        ...
        sprintf_P(public_buf_m, PSTR("G1 X%s Y%s F2000"),
          dtostrf(PrintJobRecovery::info.current_position.x, 1, 3, str_1),
          dtostrf(PrintJobRecovery::info.current_position.y, 1, 3, str_2)
        );
        gcode.process_subcommands_now(public_buf_m);
