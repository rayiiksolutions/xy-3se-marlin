...
void GcodeSuite::M600() {
  ...
  const int beep_count = parser.intval('B', -1
    ...
    screenFilament();
    #endif
	screenPopIndex(POP_FILAMENT_OUT);
  if (pause_print(retract, park_point, unload_length, true DXC_PASS)) {
    if(messageIndexNow == POP_FILAMENT_OUT)screenCloseMessageBox();
    #if ENABLED(MMU2_MENUS)
      mmu2_M600();
  ...
  else if(messageIndexNow == POP_FILAMENT_OUT)screenCloseMessageBox();
