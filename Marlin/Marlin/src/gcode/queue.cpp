...
void GCodeQueue::_commit_command(bool say_ok
  ...
  TERN_(POWER_LOSS_RECOVERY, PrintJobRecovery::commit_sdpos(index_w));
  ...
  inline void GCodeQueue::get_sdcard_commands() {
    ...
    while (length < BUFSIZE && !card_eof) {
      ...
      if (n < 0 && !card_eof) { 
        ...
        PrintJobRecovery::save(true,current_position.z + 5);
        card.endFilePrint();
        queue.clear();
        globalStop = 2;
        quickstop_stepper();
        screenPopIndex(POP_READFILE_FAIL,screenReadFailAfter);
        break;
      }
      ...
      if (is_eol || card_eof) {
        ...
        if (!process_line_done(sd_input_state, command_buffer[index_w], sd_count)) {
          ...
          #if ENABLED(POWER_LOSS_RECOVERY)
            PrintJobRecovery::cmd_sdpos = card.getIndex();     // Prime for the NEXT _commit_command
          #endif
        }
