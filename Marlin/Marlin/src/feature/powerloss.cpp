...
// PrintJobRecovery recovery;//YSZ_WORK
...
void PrintJobRecovery::changed() {
  if (!enabled){
    ...
    && my_print_status != PRINT_START_UPDATE1 && my_print_status !=PRINT_START_UPDATE2
  )
  save(current_position,true);
}
...
void PrintJobRecovery::load() {
  if (exists()){
    IFlash::read(POS_INTERRUPT + 2,&info,sizeof(info));
  }
  debug(PSTR("Load"));
}
...
void PrintJobRecovery::resume() {
  ...
  const xyze_long_t pos = info.pos;
  float zposition = pos.z * Planner::steps_to_mm[Z_AXIS],xpos,ypos;
  ...
  // Recover volumetric extrusion state
    ...
    if(globalStop > 0)return;//加热过程中出现异常,退出
  #endif
  ...
  if (dxc_is_duplicating()){
    extruder_duplication_enabled = true;//这里置true,否则下面的回抽头二不执行
    stepper.set_directions();
  }
  #endif
  if(info.zraise - zposition < 5) //如果已经抬升,说明断电之前已经回抽,这里不必再回抽
  ...
  // Reset E, raise Z, home XY...
    ...
    if(info.zraise - zposition < 5) {
      planner.synchronize();
      current_position.z += 5;
      line_to_current_position();
    }
    #if ENABLED(DUAL_X_CARRIAGE)
    if(dual_x_carriage_mode == DXC_AUTO_PARK_MODE)delayed_move_time = 0;//这里必须置0,否则复位会在idle中止
    #endif
    gcode.process_subcommands_now_P(PSTR(
    ...
    #if ENABLED(DUAL_X_CARRIAGE) // 因为G28后,双头这里会变成true,移回XY后,会执行park动作,暂停不需要这个动作
    if(dual_x_carriage_mode == DXC_AUTO_PARK_MODE)active_extruder_parked = false;
    #endif
  ...
  xpos = pos.x * Planner::steps_to_mm[X_AXIS];
  ypos = pos.y * Planner::steps_to_mm[Y_AXIS];
  sprintf_P(cmd, PSTR("G1 X%s Y%s F3000"),
    dtostrf(xpos, 1, 3, str_1),
    dtostrf(ypos, 1, 3, str_2)
  );
  gcode.process_subcommands_now(cmd);
  ...
  if(info.zraise - zposition < 5)current_position.z = info.zraise + 5;
  else current_position.z = info.zraise;
  sync_plan_position();
  sprintf_P(cmd, PSTR("G1 Z%s F2000"),dtostrf(zposition, 1, 3, str_1));
  gcode.process_subcommands_now(cmd);

  unscaled_e_move(PAUSE_PARK_RETRACT_LENGTH, feedRate_t(PAUSE_PARK_RETRACT_FEEDRATE));//抽回
  current_position.e = pos.e * Planner::steps_to_mm[E_AXIS];
  sync_plan_position();
