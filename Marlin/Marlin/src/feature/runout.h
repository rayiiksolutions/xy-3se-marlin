...
class TFilamentMonitor : public FilamentMonitorBase {
  ...
  public:
    ...
    static inline void run() { //YSZ-COMMENT:runout.run()
    ...
    static inline uint8_t outWhere(){
      uint8_t t = 0,s = sensor.poll_runout_states();
      for(; t < NUM_RUNOUT_SENSORS; t++)if(s & (1 << t))break;
      if(t == NUM_RUNOUT_SENSORS)return 0xFF;
      return t + 1;
    }
};
  ...
  class FilamentSensorSwitch : public FilamentSensorBase {
    private:
      static inline bool poll_runout_state(const uint8_t extruder) {
        if(endstops.abort_enabled())return false; //在复位期间,不进行断料检测,因为双头复位期间要切换active_extruder,但是,当第二个头不用时,也就没料,会报断料
