...
#endif
#ifdef THROAT_FAN
#define THROAT_DELAY_RET_TIME 2000
uint8_t Temperature::throatFanSt[HOTENDS] = {0};
static uint32_t throatFanTime[HOTENDS] = {0};
#endif
...
// public:
  ...
  void Temperature::set_fan_speed(uint8_t target, uint16_t speed) {
    ...
    #ifdef THROAT_FAN
    if(target >= THROAT_FAN && speed == 0){
      if(throatFanSt[target - THROAT_FAN] == 1){
        throatFanSt[target - THROAT_FAN] = 2;
        throatFanTime[target - THROAT_FAN] = millis() + THROAT_DELAY_RET_TIME;
      }
    }
    #endif
        ...
        // Make sure heating is actually working
          ...
          if (BOTH(WATCH_BED, WATCH_HOTENDS) || isbed == DISABLED(WATCH_HOTENDS)) {
            if (!heated) {                                            // If not yet reached target...
              if (current_temp > next_watch_temp || current_temp > EXTRUDE_MINTEMP) {                   // Over the watch temp?
...
void Temperature::manage_heater() {
  ...
  #ifdef THROAT_FAN
  if(degHotend(0) > 50){ //YSZ_WORK
    if(fan_speed[THROAT_FAN] == 0){
      if(throatFanSt[0] == 0 || (throatFanSt[0] == 2 && millis() > throatFanTime[0])){
        throatFanSt[0] = 1;
        fan_speed[THROAT_FAN] = 255;
      }
    }
  }
  else{
    if(throatFanSt[0] == 1){
      throatFanSt[0] = 0;
      fan_speed[THROAT_FAN] = 0;
    }
  }
  #if HOTENDS > 1
  if(degHotend(1) > 50){ //YSZ_WORK
    if(fan_speed[THROAT_FAN + 1] == 0){
      if(throatFanSt[1] == 0 || (throatFanSt[1] == 2 && millis() > throatFanTime[1])){
        throatFanSt[1] = 1;
        fan_speed[THROAT_FAN + 1] = 255;
      }
    }
  }
  else {
    if(throatFanSt[1] == 1){
      throatFanSt[1] = 0;
      fan_speed[THROAT_FAN + 1] = 0;
    }
  }
  #endif
  #if HOTENDS > 2
  if(degHotend(2) > 50){ //YSZ_WORK
    if(fan_speed[THROAT_FAN + 2] == 0){
      if(throatFanSt[2] == 0 || (throatFanSt[2] == 2 && millis() > throatFanTime[2])){
        throatFanSt[2] = 1;
        fan_speed[THROAT_FAN + 2] = 255;
      }
    }
  }
  else {
  ...
  millis_t ms = millis();
    ...
    HOTEND_LOOP() {
      ...
      temp_hotend[e].soft_pwm_amount = (temp_hotend[e].celsius > temp_range[e].mintemp || is_preheating(e)) && temp_hotend[e].celsius < temp_range[e].maxtemp ? (int)get_pid_output_hotend(e) >> 1 : 0;
        ...
        if (watch_hotend[e].next_ms && ELAPSED(ms, watch_hotend[e].next_ms)) {  // Time to check this extruder?
          if (degHotend(e) < watch_hotend[e].target && degHotend(e) < EXTRUDE_MINTEMP) {                          // Failed to increase enough?
            TERN_(DWIN_CREALITY_LCD, DWIN_Popup_Temperature(0));
    ...
    filwidth.update_volumetric();
      ...
      if (watch_bed.elapsed(ms)) {        // Time to check the bed?
        if (degBed() < watch_bed.target && degBed() < 45) {                              // Failed to increase enough?
          TERN_(DWIN_CREALITY_LCD, DWIN_Popup_Temperature(0));
...
void Temperature::init() {
  ...
  delay(250);
    ...
    #define _TEMP_MAX_E(NR) do{ \
      const int16_t tmax = _MIN(heater_maxtemp[NR], TERN(HEATER_##NR##_USER_THERMISTOR, 2000, (int16_t)pgm_read_word(&HEATER_ ##NR## _TEMPTABLE[HEATER_ ##NR## _SENSOR_MAXTEMP_IND].celsius) - 1)); \
    ...
    LOOP_L_N(e, COUNT(temp_dir)) {
      ...
      if (tdir) {
        ...
        if (heater_on && rawtemp < temp_range[e].raw_min * tdir && !is_preheating(e)) {
          ...
          #endif
              min_temp_error((heater_id_t)e);//YSZ-COMMENT:传感器失效,可以在这里检测
        }
        #ifdef MAX_CONSECUTIVE_LOW_TEMPERATURE_ERROR_ALLOWED
          else
...
void Temperature::tick() {
  ...
  void Temperature::print_heater_states(const uint8_t target_extruder
    ...
    bool Temperature::wait_for_hotend(const uint8_t target_extruder, const bool no_wait_for_cooling/*=true*/
      ...
      } while (wait_for_heatup && TEMP_CONDITIONS && globalStop == 0);
      if (wait_for_heatup) {
        wait_for_heatup = false;
        #if ENABLED(DWIN_CREALITY_LCD)
    ...
    bool Temperature::wait_for_bed(const bool no_wait_for_cooling/*=true*/
      ...
      } while (wait_for_heatup && TEMP_BED_CONDITIONS && globalStop == 0);
