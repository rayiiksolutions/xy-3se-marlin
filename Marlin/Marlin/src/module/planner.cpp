...
void Planner::endstop_triggered(const AxisEnum axis) {
  ...
  if(Buzzer::buffer.isEmpty())buzzer.tone(200,800);//这里在触碰限位开关时会多次调用,它本身会调用idle(),由于本函数是在stepper.isr中数函数中进行,所以,不能等待,需判空再执行tone
...
bool Planner::_buffer_steps(const xyze_long_t &target
  ...
  block_t * const block = get_next_free_block(next_buffer_head);//YSZ-COMMENT:这里执行idle()
...
bool Planner::_populate_block(block_t * const block, bool split_move,
  ...
  TERN_(POWER_LOSS_RECOVERY, block->sdpos = PrintJobRecovery::command_sdpos());
  block->pos = position;
