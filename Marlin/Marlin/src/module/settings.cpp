...
#if PROJ_MKS_ROBIN_PRO
#include "../lcd/extui/lib/mks_ui/pic_manager.h"
#endif
...
#define SHOW_MY_VAR_ADDR  0 //YSZ-GOGO
...
typedef struct SettingsDataStruct {
  char      version[4];                                 // Vnn\0
    ...
    BYTE dual_x_carriage_mode;
    #endif
    TouchCalibrate_t para_calibration;
...
#include "../core/debug_out.h"
  ...
  #define EEPROM_WRITE(VAR)       do{AT24CXX_Write(eeprom_index, &VAR, sizeof(VAR));                       }while(0)
  #define EEPROM_READ(VAR)        do{AT24CXX_Read(eeprom_index, (uint8_t*)&VAR, sizeof(VAR),!validating);  }while(0)
  #define EEPROM_READ_ALWAYS(VAR) do{AT24CXX_Read(eeprom_index, (uint8_t*)&VAR, sizeof(VAR));               }while(0)
  ...
  bool MarlinSettings::save() {
    ...
    TERN(FLASH_EEPROM_EMULATION, EEPROM_SKIP, EEPROM_WRITE)(ver);
      ...
      #if SHOW_MY_VAR_ADDR
      SERIAL_ECHO("my_lang-addr:");
      SERIAL_ECHOLN(eeprom_index);
      #endif
      EEPROM_WRITE(my_lang);
       #if SHOW_MY_VAR_ADDR
      SERIAL_ECHO("my_theme-addr:");
      SERIAL_ECHOLN(eeprom_index);
      #endif
       EEPROM_WRITE(my_theme);
      #if ENABLED(DUAL_X_CARRIAGE)
      #if SHOW_MY_VAR_ADDR
      SERIAL_ECHO("dual_x_mode-addr:");
      SERIAL_ECHOLN(eeprom_index);
      #endif
      EEPROM_WRITE(dual_x_carriage_mode);
      #endif
      #if SHOW_MY_VAR_ADDR
      SERIAL_ECHO("calibration-addr:");
      SERIAL_ECHOLN(eeprom_index);
      #endif
      EEPROM_WRITE(yWindows::para_calibration);
    ...
    //
    {
        ...
        #if SHOW_MY_VAR_ADDR
        SERIAL_ECHO("hotends_offset-addr:");
        SERIAL_ECHOLN(eeprom_index);
        #endif
    ...
    //
    {
      ...
      const bool recovery_enabled = TERN(POWER_LOSS_RECOVERY, PrintJobRecovery::enabled, ENABLED(PLR_ENABLED_DEFAULT));
  ...
  void MarlinSettings::first_load() 
  {
        static bool loaded = false;
        if (!loaded && load()) loaded = true;
        #if ENABLED(DUAL_X_CARRIAGE)
        switch(dual_x_carriage_mode){
        case DXC_FULL_CONTROL_MODE:gcode.process_subcommands_now_P(PSTR("M605 S0"));break;
        case DXC_AUTO_PARK_MODE   :gcode.process_subcommands_now_P(PSTR("M605 S1"));break;
        case DXC_DUPLICATION_MODE :gcode.process_subcommands_now_P(PSTR("M605 S2 X150 R0"));break;
        case DXC_MIRRORED_MODE    :
          gcode.process_subcommands_now_P(PSTR("M605 S2 X150 R0"));
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      float dummyf = 0;
        ...
        SERIAL_ECHOLN(eeprom_index);
        #endif
        EEPROM_READ(my_theme);
        #if ENABLED(DUAL_X_CARRIAGE)
        #if SHOW_MY_VAR_ADDR
        SERIAL_ECHO("dual_x_mode-addr:");
        SERIAL_ECHOLN(eeprom_index);
        #endif
        EEPROM_READ(dual_x_carriage_mode);
        #endif
        #if SHOW_MY_VAR_ADDR
        SERIAL_ECHO("calibration-addr:");
        SERIAL_ECHOLN(eeprom_index);
        #endif
        EEPROM_READ(yWindows::para_calibration);
      ...
      //
      {
        _FIELD_TEST(recovery_enabled);
        #if ENABLED(POWER_LOSS_RECOVERY)
          const bool &recovery_enabled = PrintJobRecovery::enabled;
        #else
      ...
      if (eeprom_error) {
        ...
        DEBUG_ECHO_START();
        DEBUG_ECHO(version);
        DEBUG_ECHOLNPAIR(" stored settings retrieved (", eeprom_index - (EEPROM_OFFSET), " bytes)");
      }
  ...
  bool MarlinSettings::load() {
    ...
    reset();
    queue.enqueue_one_P("M500"); //如果是初始状态,全写一次
...
void MarlinSettings::reset() {
  ...
  // Power-Loss Recovery
  //
  TERN_(POWER_LOSS_RECOVERY, PrintJobRecovery::enable(ENABLED(PLR_ENABLED_DEFAULT)));
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    TERN_(CONTROLLER_FAN_EDITABLE, M710_report(forReplay));
      ...
      CONFIG_ECHO_START();
      SERIAL_ECHOLNPAIR("  M413 S", int(PrintJobRecovery::enabled));
    #endif
