...
// For M0/M1, this flag may be cleared (by M108) to exit the wait-for-user loop
  ...
  void wait_for_user_response(millis_t ms/*=0*/, const bool no_sleep/*=false*/) {
    ...
    while (wait_for_user && !(ms && ELAPSED(millis(), ms)) && globalStop == 0)
...
void startOrResumeJob() {
  ...
  inline void abortSDPrinting() {
    ...
    TERN_(POWER_LOSS_RECOVERY, PrintJobRecovery::purge());
    
    #ifdef EVENT_GCODE_SD_ABORT
      gcode.process_subcommands_now_P(EVENT_GCODE_SD_ABORT);
      gcode.process_subcommands_now_P("T0");
      gcode.process_subcommands_now_P("G92 E0");
      gcode.process_subcommands_now_P(SD_FINISHED_RELEASECOMMAND);
    #endif
  ...
  inline void finishSDPrinting() {
    if (queue.enqueue_one_P(PSTR("M1001"))) {
      if(my_print_status == PRINT_START_UPDATE1 || my_print_status == PRINT_START_UPDATE2)my_print_status = PRINT_UPDATE_END;//打印完成
      else my_print_status = PRINT_IDLE;
      #if TRONXY_PRINTING_NOWAIT
      printing_no_wait = false;
      #endif
...
void idle(TERN_(ADVANCED_PAUSE_FEATURE, bool no_stepper_sleep/*=false*/)) {
  ...
  // Handle Power-Loss Recovery
    ...
    if(globalStop > 32)
    {
      while(buzzer.state.endtime)buzzer.tick();//等待蜂鸣器结束
      delay(200);
      #if T2_STM32F103
      nvic_sys_reset();
      #elif T2_STM32F446
...
void setup() {
  ...
  #if HAS_FILAMENT_SENSOR
    SETUP_RUN(runout.setup());//YSZ-COMMENT:断料检测初始化
  #endif

  #if ENABLED(POWER_LOSS_RECOVERY)
    // SETUP_RUN(PrintJobRecovery::setup());
  #endif
...
void loop() {
  do {
    ...
    queue.advance();
    //YSZ-COMMENT:解释并执行指令,这里面也会调用idle()
    //重点是gcode.process_next_command()
    //它里面parser.parse(current_command)解析指令,process_parsed_command()执行指令
    //process_parsed_command()执行指令等待是Planner::_buffer_steps函数,移动指令几乎都调用它,它调用idle()
    endstops.event_handler();

    TERN_(HAS_TFT_LVGL_UI, printer_state_polling());
    if(globalStop > 0)globalStop--;
