...
uint16_t CardReader::longFilename[LONG_FILENAME_LENGTH];
...
void openFailed(const char * const fname) {
  ...
  screenPopIndex(POP_OPENFILE_FAIL);
}
...
void CardReader::checkautostart() {
  ...
  if (isMounted() && TERN1(POWER_LOSS_RECOVERY, !PrintJobRecovery::valid())) {
    char autoname[8];
...
const char* CardReader::diveToFile(const bool update_cwd, SdFile*& diveDir, const char * const path, const bool echo/*=false*/) {
  ...
  if (path[0] == '/') {               // Starting at the root directory?
    diveDir = &root;
    if(!diveDir->isOpen())root.openRoot(&volume);//YSZ_WORK:原固件这里没有,注意,当workDir为root时,它open后,root本身是没有open的,它们是值传递,所以,这里回到root要重新打开
    item_name_adr++;
  ...
  if (update_cwd) {
    #if 0 //YSZ_WORK:这里需要关掉原先的文件夹,才可赋新值
    workDir = *diveDir;
    #else
    if(diveDir != &workDir){
      if(workDir.isOpen())workDir.close();
      workDir = *diveDir;
    }
    #endif
...
void CardReader::fileHasFinished() {
  planner.synchronize();
  file.close();
  gcode.process_subcommands_now_P("T0");
...
#if ENABLED(POWER_LOSS_RECOVERY) && 0

  bool CardReader::jobRecoverFileExists() {
    const bool exists = PrintJobRecovery::file.open(&root, PrintJobRecovery::filename, O_READ);
    if (exists) PrintJobRecovery::file.close();
    return exists;
  }

  void CardReader::openJobRecoveryFile(const bool read) {
    if (!isMounted()) return;
    if (PrintJobRecovery::file.isOpen()) return;
    if (!PrintJobRecovery::file.open(&root, PrintJobRecovery::filename, read ? O_READ : O_CREAT | O_WRITE | O_TRUNC | O_SYNC))
      SERIAL_ECHOLNPAIR(STR_SD_OPEN_FILE_FAIL, PrintJobRecovery::filename, ".");
    else if (!read)
      echo_write_to_file(PrintJobRecovery::filename);
  }
  ...
  void CardReader::removeJobRecoveryFile() {
    if (jobRecoverFileExists()) {
      ...
      PrintJobRecovery::file.open(&root, PrintJobRecovery::filedir, O_RDONLY);
      PrintJobRecovery::file.rmRfStar();
      PrintJobRecovery::file.close();
