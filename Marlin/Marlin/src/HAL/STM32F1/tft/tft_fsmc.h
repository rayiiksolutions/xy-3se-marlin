...
class TFT_FSMC {
  ...
  public:
    ...
    #if T2_STM32F103 //YSZ_WORK:这个函数和下面的函数重载时,uint32_t和uint16_t很容易分不清类型,编译器会报错,只能保留一个
    static void WriteMultiple(uint16_t Color, uint32_t Count) {
      static uint16_t Data; Data = Color;
      while (Count > 0) {
        //YSZ_WORK:github源码是CIRC模式,但这里其实是0,否则死循环
        TransmitDMA(/*DMA_CIRC_MODE*/0, &Data, Count > 0xFFFF ? 0xFFFF : Count);
